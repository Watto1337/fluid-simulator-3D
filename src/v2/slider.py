...
class Slider():
    sensitivity = 10
    text = pygame.font.SysFont("timesnewroman", 15)

    def __init__(self, val1, val2, width, pos, color, name):
        if val1 < val2:
            self.min = val1
            self.max = val2
            self.reversed = False
        ...
        self.name = Slider.text.render(name, True, color)
    ...
    def draw(self, display, mousePos, mouseDown):
        ...
        pygame.draw.line(display, self.color, self.pos, (self.pos[0] + self.width, self.pos[1]), 2)
        pygame.draw.circle(display, self.color, (self.pos[0] + self.width * (self.n - self.min) / (self.max - self.min), self.pos[1]), 5)
